...
| 0022 | `noise.version` 2 = render-time noise (canvas text drawn at a seeded scale, WebGL `gl_Position` nudged via `shaderSource`; nothing altered on read), version 1 = legacy read-time noise, `canvas_threshold = 0` disables canvas noise; per-context WebGL extension allowlists (`webgl.extensions` = WebGL2, `webgl.extensions_webgl1`) enforced in both `getSupportedExtensions` and `getExtension`; audio noise applied once at `OfflineAudioContext` completion, scaled by `noise.audio_amplitude` | `0022-chronium-render-time-noise-v2-per-context-WebGL-exte.patch` |
